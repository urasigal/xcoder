...
public class BroadcasterIntelTrannscodeStreamTest extends BaseTest {
	...
	throws InterruptedException {
		...
		"rec_history", "rec_duration", "src", "ap", "use_hw", "ll", "all_pids", "bit", "profile_name", "mode", "testid" },
		...
		Assert.assertEquals(((BroadcasterTrannscodeStreamDriver) testDriver) .testIMPL( userName,  userPass,  login_ip,  uiport,  type, id,
		 matrix,  max_outputs,  mcast_out,  time_shift,  old,  fast_connect,  kompression,  enc_type,  enc_key,
		 rec_history,  rec_duration,  src, ap, use_hw, ll,  all_pids, bit,  profile_name, mode), "Stream " + "'" + id + "'" + " added.");
	}
